...
const createcollege = async function (req, res) {
  try {
    ...
    if (!validFullName(fullName.trim()))
      return res
        .status(400)
        .send({ status: false, msg: "Enter valid fullName" });
    if (!logoLink)
...
const getCollege = async function (req, res) {
  try {
    ...
    if (!college)
      return res
        .status(404)
        .send({ status: false, msg: "college with this name does not exits" });

    const interns = await InternModel.find({ collegeId: college._id }).select({
      name: 1,
      email: 1,
      mobile: 1,
    });
    ...
    if (interns.length == 0)
      return res
        .status(400)
        .send({ status: false, msg: "no intern found in this college" });
